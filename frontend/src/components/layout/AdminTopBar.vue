...
const isOnProfile = computed(() => route.name === 'admin-profile')
...
function goToProfile() {
  if (!isOnProfile.value) router.push({ name: 'admin-profile' })
}
</script>

<template>
  <header class="sticky top-0 z-30 bg-surface/90 backdrop-blur-sm border-b border-border safe-area-top">
    <div class="flex items-center justify-between h-14 px-3 md:h-16 md:px-6">
      ...
      <div class="flex items-center gap-2 flex-shrink-0 relative" data-topbar-account>
        <button
          ...
          :aria-label="`Admin profile: ${lastName || 'Profile'}`"
          @click="goToProfile"
        >
          <span
